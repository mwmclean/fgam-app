...
shinyServer(function(input, output, session) {
  ...
  output$n.test <- renderUI({
    ...
    sliderInput("n.testing", "Number of samples to use for test set:", Nstart, min = 1, max = Nmax, step = 1)
  ...
  fitFLM <- reactive(function(){
    ...
    isolate({
      ...
      if(input$pred){
        ...
        dat$L.X <- dat$X/J
        #  print(class(dat$L.X))
        dat$tmat <- matrix(seq(0, 1, l=J), N-input$n.testing, J, byrow=TRUE)
...
#    input$fitaction
  ...
  fitFGAM <- reactive(function(){
                      ...
                      isolate({
                        ...
                        if(input$pred){
                          ...
                          dat$X <- dat$X[-test.samps, ]
                          dat$y <- dat$y[-test.samps]
                          dat$L <- matrix(1/J, N-input$n.testing, J)
                          dat$tmat <- matrix(seq(0, 1, l=J), N-input$n.testing, J, byrow=TRUE)
                      ...
                      fit <- with(dat, switch(input$sptypeFGAM,
                                                ...
                                                'Fixed'=gam(y~te(X, tmat, by=L, k=k), method = 'REML',
                                                            sp=c(exp(get('xsp', env=globalenv())),
                                                                 exp(get('tsp', env=globalenv()))))))
                      assign('fit.fgam', fit, globalenv())
                      fit
  ...
  output$prederror <- renderPrint({
    ...
    isolate({
      ...
      #get('fit.fgam', globalenv())
      #get('fit.flm', globalenv())
      ...
      predFGAM <- predict(fit.fgam, newd)[seq_len(input$n.testing)]
    ...
    isolate({
      ...
      if ('RLRT2'%in%input$tests){
        ...
        if (is.na(res[1]) && is.na(res[2])){
          ...
          res <- mean(res[1:2])  # signif(max(res[1]*2, 2*res[2]),2)
        }
        cat('Using RLRT2:\np-value is approximately ', res,'\n')
      }
