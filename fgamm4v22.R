...
testfun <- function(y,X,tvals=seq(0,1,l=ncol(X)), fixed.effects, family=gaussian(),
  ...
  if (numeric_version(packageVersion('lme4')) < 1){
    stop('Upgrade to the latest version of lme4')
  }
  ...
  for(i in 1:1){
    ...
    b$reTrms$Zt[ii, ] <-   as(t(Design[,names(parinds)==sn[i]]),'dgCMatrix')
    b$reTrms$cnms[[k]] <- snames[i]
  }

  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  m1 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ...
  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  m2 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ...
  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  mA1 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ...
  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  mA2 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ...
  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  m0 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)

 # browser()
  var1 <- VarCorr(mA1)[[1]]  #onet.Zx
  var1.0 <- VarCorr(m1)[[1]]
  var2 <- VarCorr(mA2)[[1]]  #Zt.Zx
  var2.0 <- VarCorr(m2)[[1]]
  var0 <- VarCorr(m0)[[1]]
  if(var0==0)
    return(rep(NA,5))
...
#  ret$lme4fit <- do.call(lme4:::lmer_finalize,tempfit)
  ret$lme4fit <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ret$bbx <- bbx
  ret$bbt <- bbt
...
#   }
 ...
 # names(tempfit$fr$fixef) <- c('1','x','x*t')
  ...
  m2 <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
 #  browser()
  var1.0 <- VarCorr(m1)[[1]]  # onet.Zx
  var2.0 <- VarCorr(m2)[[1]]  # Zt.Zx
...
TestKnowEqualVC <- function(y,X,tvals=seq(0,1,l=ncol(X)), family=gaussian(),
                          splinepars=list(k=c(8,8), m=list(c(2,2),c(2,2)), extendXrange=.01),
                          REML=TRUE, oracle=0, formratio=FALSE, sig2FLM=NULL, ratio=NULL,
                          psplines=TRUE, removeCon = FALSE, no.nuisvc.test.if.0est = TRUE, fixed.effects){
  if (numeric_version(packageVersion('lme4')) < 1){
    stop('Upgrade to the latest version of lme4')
  }
...
#     }
  ...
  #browser()
  # m <- do.call(lme4:::lmer_finalize,tempfit)
  m <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ###########################################################################################
  # 2)
  ...
  devfun <- do.call(lme4::mkLmerDevfun, b)
  opt <- lme4::optimizeLmer(devfun, optimizer = "bobyqa")
  mA <- lme4::mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
 ...
 # browser()
  var1 <- VarCorr(m)[[1]]  #onet.Zx
  var2 <- VarCorr(mA)[[1]]  #Zt.Zx
  var0 <- VarCorr(m0)[[1]]
  # browser()
  if(var0==0){
    if(!no.nuisvc.test.if.0est || var1==0){
      return(rep(NA,4))
    }else{
      pval <- exactRLRT(m)$p
      return(c(pval, pval < .1, pval < .05, pval < .01))
    }
  }
  if(var1==0 || var2==0){
    return(c(NA,0,0,0))
...
TestBoot <- function(y,X,tvals=seq(0,1,l=ncol(X)), family=gaussian(),
                          splinepars=list(k=c(8,8), m=list(c(2,2),c(2,2)), extendXrange=.01),
                          REML=TRUE, oracle=0, formratio=FALSE, sig2FLM=NULL, ratio=NULL,
                          psplines=TRUE, removeCon = FALSE, BOOT=TRUE, n.sims.boot=500, boot.semipar=FALSE,
                          boot.condition.on.b1=TRUE, fixed.effects, cl=cl){
  if (numeric_version(packageVersion('lme4')) < 1){
    stop('Upgrade to the latest version of lme4')
  }
...
#  browser()
  ...
  form <-as.formula(form)
#  for lme4 version < 1.04
#  tempfit <- lmer(form, data=dat, REML=REML)
...
#   }
  ...
  # tn <- names(tempfit$FL$fl)
  # tn <- tn[attr(tempfit$FL$fl,'assign')]
  b <- lFormula(form, data = dat)
  tn <- names(b$reTrms$cnms)
  ind <- 1:length(tn)
  ind <- 1:length(tn)
  sn <- dnames[2:4]
  snames <- c('FLM','ntpx', 'ptpx')
  for(i in 1:3){
  ...
  mA <- mkMerMod(environment(devfun), opt, b$reTrms,
                  fr = b$fr)
...
#     for compatibility for lme4 version < 1.04
#   tn <- names(tempfit$FL$fl)
#   tn <- tn[attr(tempfit$FL$fl,'assign')]
  ...
  for(i in 1:1){
    k <- ind[sn[i] == tn]
    ii <- (b$reTrms$Gp[k] + 1):b$reTrms$Gp[k + 1]
    b$reTrms$Zt[ii, ] <-   as(t(Design[,names(parinds)==sn[i]]),'dgCMatrix') # as(t(G$random[[i]]), "dgCMatrix")
    b$reTrms$cnms[[k]] <- snames[i]  #attr(G$random[[i]], "s.label")
#     for compatibility for lme4 version < 1.04
#     k <- ind[sn[i]==tn]
#     tempfit$FL$trms[[k]]$A <- tempfit$FL$trms[[k]]$Zt <-
#       as(t(Design[,names(parinds)==sn[i]]),'dgCMatrix')
#     attr(tempfit$FL$trms[[k]]$ST,'dimnames') <- list(snames[i],snames[i])
  ...
  devfun <- do.call(mkLmerDevfun, b)
  opt <- optimizeLmer(devfun, optimizer = "bobyqa")
  m0 <- mkMerMod(environment(devfun), opt, b$reTrms, fr = b$fr)
  ...
  if(!BOOT){
    ...
    # browser()
    # from ?simulate-mer in lme4 version <1.04
#     pboot <- function(m0,m1) {
#       s <- simulate(m0)
#       L0 <- logLik(refit(m0,s))
#       L1 <- logLik(refit(m1,s))
...
#
   ...
   # pboot.res <- try(replicate(n.sims.boot, pboot(m0, mA)),FALSE)
     ...
     if(boot.semipar){
       samps <- replicate(n.sims.boot, fitted(m0) + sample(residuals(m0, "response")), simplify = 'matrix')
     }else{
       samps <- simulate(m0, n.sims.boot, use.u=boot.condition.on.b1)
     }
     #pboot.res <- apply(samps, 2, function(x) 2*(logLik(refit(mA, x))-logLik(refit(m0, x))))
    pboot.res <- parApply(cl, samps, 2, function(x) 2*(logLik(lme4:::refit(mA, x))-logLik(lme4:::refit(m0, x))))
  #  pboot.res <- try(bootMer(x=m0, FUN=function(x)2*(logLik(refit(m0, x))-logLik(refit(mA, x))) , nsim=n.sims.boot), FALSE)
    if (class(pboot.res)=='try-error'){
      return(rep(NA, 4))
    }else{
      p.val <- mean(obsdev < pboot.res)
